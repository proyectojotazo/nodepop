...
const splitParams = (queryParams = {}) => {
  ...
  Object.keys(queryParams).forEach((key) => {
    if (paramsQueryToCheck.includes(key)) {
      // query
      query[key] = queryParams[key]
    } else if (paramsOptionsToCheck.includes(key)) {
      // optionals
      if (key === 'start') {
        // Pasaremos la propiedad que nos viene como 'start' a 'skip' para filtrar
...
const getPrecioQuery = (precioParam = '') => {
  ...
  if (longParam === 1) {
    // Cuando precioParam sea '12'
    precioQuery = Number(paramSplitted[0])
  } else {
    // Cuando precioParam sea cualquier otro caso
    if (paramSplitted[0] === '') {
      // si paramSplitted = ['', '200'] o precioParam = -200
      precioQuery = { $lte: Number(paramSplitted[1]) }
    } else if (paramSplitted[1] === '') {
      // si paramSplitted = ['12', ''] o precioParam = 12-
      precioQuery = { $gte: Number(paramSplitted[0]) }
    } else {
      // si paramSplitted = ['12', '200] o percioParam = 12-200
      precioQuery = {
        $gte: Number(paramSplitted[0]),
        $lte: Number(paramSplitted[1]),
      }
    }
  }
